...
export const SVG_BUILDER: SvgBuilder = {
  ...
  createDefsPattern: (parent: SVGElement | HTMLElement | null,
    ...
    if (result) {
      ...
      result.setAttribute('patternUnits', `${currentOption.patternUnits ==
                                             undefined ? 'userSpaceOnUse' : currentOption.patternUnits}`);
    }
    return result;
  },
  // ========================================================================
  ...
  createLine: (vector: Vector,
    ...
    if (result) {
      result.setAttribute('d', SVG_BUILDER.renderLine(vector));
      return result;
    } else {
      return null;
    }
  ...
  renderLine: (vector: Vector): string => {
    ...
    return `M ${vector.start.x},${vector.start.y} ${vector.end.x},${vector.end.y}`;
  },
  // ========================================================================
  // createCurve
  // ========================================================================
  ...
  createRect: (parent: SVGElement|null|undefined, option?: RectOption): SVGElement | null => {
    ...
    if (currentOption.round) {
      result.setAttribute("r", '' + currentOption.round);
    }
  ...
  createCircle: (parent: SVGElement|null|undefined, option?: CircleOption): SVGElement | null => {
    ...
    if (currentOption.height!=undefined) {
      result.setAttribute("height", '' + currentOption.height);
    }
    if (currentOption.width!=undefined) {
      result.setAttribute("width", '' + currentOption.width);
    }
    if (currentOption.round!=undefined) {
      result.setAttribute("r", '' + currentOption.round);
    }

    if (currentOption.x!=undefined) {
      result.setAttribute("cx", '' + currentOption.x);
    }
    if (currentOption.y!=undefined) {
      result.setAttribute("cy", '' + currentOption.y);
    }
...
export const SVG_TRANSFORM: SvgTransform = {
  ...
  removeClass: (node: SVGElement | HTMLElement | null | undefined, styleclass: string): void => {
    ...
    const values = currentClass.split(' ').map(v => v.trim()).filter(v => v != styleclass);
  ...
  addClass   : (node: SVGElement | HTMLElement | null | undefined, styleclass: string): void => {
    ...
    const currentClass = node.getAttribute("class");
    if (!currentClass) {
      node.setAttribute('class', styleclass.trim());
      return;
    }

    const values = currentClass.split(' ').map(v => v.trim());
    if (!values.includes(styleclass)) {
      values.push(styleclass)
    }
